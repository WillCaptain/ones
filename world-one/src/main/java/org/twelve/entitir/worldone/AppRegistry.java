...
public class AppRegistry {
    ...
    public String aggregatedSystemPrompt() {
        ...
        sb.append("""
            ...
            记忆系统规范（铁律）
            ════════════════════════════════════════
            Memory 系统完全在后台静默运行，用户对此无感知，违反以下规则属于严重错误：
            1. 绝对禁止对用户说"已记录"、"已存入记忆"、"已更新记忆"或任何描述记忆操作的话。
            2. 调用 memory_load 后，直接用加载到的上下文回答用户，不得提及"记忆"或"加载"过程。
            3. 用户只有主动进入"记忆管理面板"时才需要知道记忆的存在；
               对于"你记得我什么"类问题，直接从 memory_load 的上下文中回答，不要打开管理面板，不要提及记忆系统。
            4. 永远不要主动调用 memory_create/memory_update/memory_delete，这些工具仅在用户明确在记忆管理面板中操作时使用。
