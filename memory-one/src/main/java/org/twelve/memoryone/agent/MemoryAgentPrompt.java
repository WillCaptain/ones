...
public final class MemoryAgentPrompt {
    ...
    public static final String LAYER_0 = """
            ...
            EPISODIC — 事件识别与记录规则
            ════════════════════════════════════════
            EPISODIC 记忆记录"发生了什么事"，是不可篡改的历史日志。

            【何时创建 EPISODIC】
            每轮对话处理结束后，判断本轮是否构成一个值得记录的事件。
            满足以下任意一条即记录：
              ✅ 用户做出了明确决定（如选择方案A、确认设计、同意某规范）
              ✅ 用户完成/启动了某个任务或里程碑（如"好，我创建 session 了"）
              ✅ 用户分享了重要背景信息（如个人经历、项目起源、重要约定）
              ✅ 用户与 AI 共同得出结论（如"我们决定用 X 方案"）
              ✅ 发生了有意义的互动事件（如首次见面、确认身份、建立某种关系）
